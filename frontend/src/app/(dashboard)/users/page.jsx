      ...
      <div className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-2xl shadow-sm rounded-[32px] overflow-hidden border border-white/50 dark:border-white/10">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {users.map((user) => (
              <tr key={user.id} className={clsx("transition-colors", !user.isActive && "bg-gray-50 dark:bg-gray-800/50 opacity-50")}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900 dark:text-white flex items-center gap-2">
                    <span className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold uppercase">{user.name[0]}</span>
                    {user.name}
                  </div>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  {user.role !== 'admin' && (
                    user.isActive ? (
                      <button onClick={() => handleDeactivate(user.id)} className="text-red-600 hover:text-red-900 dark:hover:text-red-400 flex items-center gap-1 justify-end ml-auto">
                        <UserX className="h-4 w-4" /> Deactivate
                      </button>
                    ) : (
                      <button onClick={() => handleActivate(user.id)} className="text-green-600 hover:text-green-900 dark:hover:text-green-400 flex items-center gap-1 justify-end ml-auto">
            ...
            <div className="relative bg-white/60 dark:bg-gray-800/60 backdrop-blur-3xl rounded-[32px] border border-white/50 dark:border-white/10 px-4 pt-5 pb-4 text-left shadow-2xl transform transition-all sm:my-8 w-full max-w-lg sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Staff members will have access to create and manage customers and invoices, but cannot view reports or manage other users.</p>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input {...register('email', { required: true })} type="email" className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent !text-gray-900 dark:!text-white" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Temporary Password</label>
                      <input {...register('password', { required: true, minLength: 8 })} type="text" className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-transparent !text-gray-900 dark:!text-white" />
