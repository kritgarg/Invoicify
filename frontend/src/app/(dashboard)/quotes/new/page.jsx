...
import { Trash2, ArrowLeft, Loader2, CheckCircle2 } from 'lucide-react';
...
export default function NewQuotePage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Error saving quote');
    } finally {
      ...
      <div className="bg-white/70 backdrop-blur-3xl shadow-sm rounded-[32px] border border-gray-900/10 p-8">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="pt-6 border-t flex justify-end gap-3">
             <Link href="/quotes" className="px-5 py-2.5 bg-white border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">
               Cancel
             </Link>
